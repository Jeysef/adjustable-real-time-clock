...
static void readSDA(void) {
    GPIO_Init(DS1302_GPIO, DS1302_SDA, GPIO_MODE_IN_PU_NO_IT);
}
...
//		DS1302_SDA = (bit)(addr & 1); 
        if (cmd & 1) {
            GPIO_WriteHigh(DS1302_GPIO, DS1302_SDA);
        } else {
            GPIO_WriteLow(DS1302_GPIO, DS1302_SDA);
        }
//		DS1302_SCK = 1;
        GPIO_WriteHigh(DS1302_GPIO, DS1302_SCLK);
		delay_us(1);
//		DS1302_SCK = 0;
...
//	DS1302_RST = 1;
    GPIO_WriteHigh(DS1302_GPIO, DS1302_RST);
...
//	DS1302_RST = 1;
	...
	for (i = 0; i < 8; i ++) 
	{
		temp >>= 1;
//		if(DS1302_SDA)
        if (GPIO_ReadInputPin(DS1302_GPIO, DS1302_SDA))
			temp |= 0x80;
//		DS1302_SCK = 1;
        GPIO_WriteHigh(DS1302_GPIO, DS1302_SCLK);
		delay_us(1);
...
void DS1302_Init(void)
{
    ...
    GPIO_Init(DS1302_GPIO, DS1302_SDA, GPIO_MODE_OUT_PP_HIGH_FAST);
    GPIO_Init(DS1302_GPIO, DS1302_RST, GPIO_MODE_OUT_PP_HIGH_FAST);
